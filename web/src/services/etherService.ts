...
export class EtherService {
  ...
  public async getProvider(): Promise<CustomProvider> {
    const metamask: MetamaskType =
      (await detectEthereumProvider()) as MetamaskType;
  ...
  public async getUserBalance(): Promise<string> {
    ...
    const balance: ethers.BigNumber = await this.provider.getBalance(
      this.userAccount
    );
  ...
  public async approveToken(
    ...
    const tx: ethers.ContractTransaction =
      await erc20Instance.functions.approve(to, bigIntAmount.toString());
    await tx.wait();
    return tx;
  }
  ...
  public async checkTokenAllowence(
    ...
    const balance: ethers.BigNumber = await erc20Instance.allowance(
      ownerAddress,
      spenderAddress
    );
